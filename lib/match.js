...
function placeOrder(db, user, req) {
  db.exec('BEGIN');
  try {
    const result = placeOrderTx(db, user, req);
    db.exec('COMMIT');
    return result;
  } catch (e) {
    db.exec('ROLLBACK');
...
// The matching body without its own BEGIN/COMMIT — reused by cash-out, which
// needs one transaction spanning the full-fill check (a partial hedge fill
// rolls back entirely).
