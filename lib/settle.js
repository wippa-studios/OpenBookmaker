...
function settleMarket(db, marketId, { winner_selection_id = null, void: voidMarket = false } = {}) {
  db.exec('BEGIN');
  try {
    ...
    if (voidMarket) {
      ...
      if (!winner_selection_id) throw httpError(400, 'winner_selection_id required (or void: true)');
      const sel = db.prepare('SELECT * FROM selections WHERE id = ? AND market_id = ?').get(winner_selection_id, marketId);
      if (!sel) throw httpError(400, 'Winner selection is not in this market');
      db.prepare("UPDATE markets SET status = 'settled', settled_at = ?, winner_selection_id = ? WHERE id = ?").run(now(), winner_selection_id, marketId);
    }
    ...
    for (const m of rows) {
      ...
      } else if (m.selection_id === winner_selection_id) {
